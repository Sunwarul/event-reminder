...
const removeImage = () => {
    router.post(`/events/${props.event.id}/remove-image`, {}, {
        preserveScroll: true,
        onSuccess: (res: any) => {
            form.image = '';
...
<template>
    <form @submit.prevent="submit" class="space-y-4 max-w-2/3 mb-5 border p-5 rounded-lg">
        ...
        <div>
            ...
            <div v-else-if="props.event.image" class="mt-2">
                <img :src="`/storage/${props.event.image}`" alt="Image Preview" class="max-h-40 rounded border" />
                <button @click.prevent="removeImage" class="mt-2 text-sm text-red-600">Remove</button>
            </div>
        </div>
        <div>
            <label class="block mb-1 font-medium">Status</label>
            <select v-model="form.status" class="w-full rounded border px-3 py-2">
                <option v-for="status in eventStatuses" :key="status.value" :value="status.value">{{ status.label }}
                </option>
            </select>
